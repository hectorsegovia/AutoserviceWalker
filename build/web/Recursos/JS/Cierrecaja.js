...
function construirJSON(ban) {
    datos = {
        bandera: ban,
        id_cierre: (document.getElementById('id_cierre').value === "") ? "0" : document.getElementById('id_cierre').value,
        monto_total: document.getElementById('total').value,
...
function limpiarcierre(){
    document.getElementById('id_cierre').value = "";
    document.getElementById('id_apertura').value = "";
...
function desabilitarcampos(){
    document.getElementById('id_cierre').disabled = true;
    document.getElementById('id_apertura').disabled = true;
...
function envio() {
    ...
    xmlhttp.open("POST", "/AutoserviceWalker/CierreCTRL");
...
function recuperarArqueoUnico() {
        ...
        if (xhr.readyState === 4) {
            if (xhr.status === 200 && xhr.status < 300)
            {
                ...
                for (i = 0; i < json.length; i++) {
                    document.getElementById('total').value = json[i].monto_total;
    ...
    xhr.open('POST', '/AutoserviceWalker/CierreCTRL');
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4) {
            if (xhr.status === 200 && xhr.status < 300)
            {
                ...
                var filas = "";
                for (i = 0; i < json.length; i++) {
                    filas += "<tr onclick=getTDcierre(this);>";
